...
extension ARSCNView {
  ...
  func hitTestWithInfiniteHorizontalPlane(_ point: CGPoint, _ pointOnPlane: SCNVector3) -> SCNVector3? {
    ...
    guard let ray = hitTestRayFromScreenPos(point), ray.direction.y > -0.03 else {
      return nil
    }

    // Return the intersection of a ray from the camera through the screen position with a horizontal plane
    // at height (Y axis).
    return SCNNode.rayIntersectionWithHorizontalPlane(rayOrigin: ray.origin, direction: ray.direction, planeY: pointOnPlane.y)
  }
